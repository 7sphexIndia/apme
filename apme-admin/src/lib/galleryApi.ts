...
export async function adminFetchEnquiries(): Promise<Enquiry[]> {
  const res = await apiFetch('/api/contact_admin.php', { method: 'GET' })
  const json = (await res.json().catch(() => ({}))) as { data?: Enquiry[]; error?: string }
